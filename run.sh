...
if $start_facade; then
  ...
  python3 -m facade_service.facade_service --host "$host" --port "$facade_service_port" &
fi
...
if $start_logging; then
  ...
  for port in "${logging_ports[@]}"; do
    ...
    hz start -c hazelcast.xml -p $hazelcast_port &
    curl --request PUT \
          --data "{\"ID\": \"hazelcast$i\", \"Name\": \"hazelcast\", \"Address\": \"$host\", \"Port\": $hazelcast_port}" \
          http://localhost:8500/v1/agent/service/register &
    python3 -c "from logging_service.logging_service import serve; serve($port)" &
...
if $start_messages; then
  ...
  for port in "${message_ports[@]}"; do
    ...
    python3 -m messages_service.messages_service --host "$host" --port "$port" &
  done
fi
